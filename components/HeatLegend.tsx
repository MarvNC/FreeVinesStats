import React from 'react';

const HEAT_LEVELS = [0, 1, 2, 3, 4, 5] as const;

const HeatLegend: React.FC = () => (
  <div className="flex items-center gap-1.5 mt-4 text-[10px] text-stone-400 dark:text-stone-500 font-medium select-none" aria-label="Color scale: less to more activity">
    <span>Less</span>
    {HEAT_LEVELS.map((i) => (
      <div
        key={i}
        className={`size-3 rounded-none bg-heat-${i} border border-white/5 dark:border-black/5`}
        aria-hidden="true"
      />
    ))}
    <span>More</span>
  </div>
);

export default HeatLegend;
